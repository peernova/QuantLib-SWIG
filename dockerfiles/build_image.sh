...
if [ "$ci" = true ]; then
  for p in amd64 arm64; do
    ...
    docker buildx build --no-cache --memory=20g --memory-swap=20g --ulimit stack=536870912 --ulimit nofile=32768 --platform linux/${p} --build-arg="cpu_arch=${p}" -t ${repo}/quantlib:${p} --build-arg="quantlib_version=$quantlib_version" -f pn.ci.quantlib.Dockerfile .
